...
try {
    ...
    $consulta = $pdo->prepare("SELECT * FROM clientes WHERE rutas = '1' ORDER BY id_cliente DESC");
    $consulta->execute();
    if ($data !== "") {
        $consulta = $pdo->prepare("SELECT * FROM clientes WHERE (rutas = '1') AND (id_cliente LIKE :filtro OR folio LIKE :filtro OR Nombre LIKE :filtro OR precio LIKE :filtro)");
        $filtro = '%' . $data . '%';
